...
#
##  execute in unix: perl term_doc.pl 10 NTFLONG vocabulary.txt
##  The first argument, here '10', is for document frequency thresholding
##  The third argument, here 'vocabulary.txt', may be either the name of a file that contains a vocabularly where the first word on each line is the token, or the name of file that the vocabulary should be written to.
##
#
#
#!/usr/local/bin/perl
...
## vjd 3/3/16: added this for optional list of vocabularly for out of sample docs
my $vocab = $ARGV[2];
...
## vjd 6/5/16: NOTE: this has been added for MID5 classification
if (-f $vocab) {
    open(INVOC, $vocab) or die "Can\'t open vocabulary file; error $!";
    print "Using file $vocab for the vocabulary.\n";
} else {
    open(VOC, ">$vocab") or die "Can\'t open vocabulary file; error $!";
    $newvocab = 1;
    print "Writing the vocabulary to a new file, $vocab\n";
}
...
## vjd 6/5/16: If we are writing a new vocabulary file
if($newvocab==1) {
    my $ki=1;
    foreach (@keepKey) {
        print VOC "$_\t$ki\n";
...
## Notice that everything loops through @keepkey, which is either created and written OR read from a file if a third argument is passed

## if vocabularly list has been passed, parsing tokens into array
if ($newvocab==0) {
    @keepKey = ();
    while ($line = <INVOC>) {
        my @words = split(/\s+/, $line);
        $line = &TrimString($words[0]);
        push(@keepKey, $line)
    }
    close(INVOC);
}

# these are the column names, or first row for a "wide" file
if (exists($outmat{"NTFWIDE"})) {
  print NTFWIDE do {
      local $" = "\t";
      "doc_id\t"."@keepKey\n";
